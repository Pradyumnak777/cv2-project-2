...
#simple_radial and shared, as same camera used, with same focal length(unsure of autozoom/autofocus. used iphone 14 base)
...
#start the actual 3d reconstruction, bundle adjustment(?)
if not os.path.exists(sparse_out):
    os.makedirs(sparse_out)
os.system(f"colmap mapper --database_path {db} --image_path {img_folder} --output_path {sparse_out}")
